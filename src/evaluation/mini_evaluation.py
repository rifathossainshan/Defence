...
def run_mini_evaluation(config):
    ...
    # 3. PCA Plot (Memory Safe replacement for UMAP)
    print("Computing PCA...")
    ...
    plt.title("Mini Evaluation: PCA of Hybrid Embeddings (50 Samples)")
    plt.tight_layout()
    plt.savefig(config["umap_path"], dpi=200)
    plt.close()
    print(f"PCA plot saved to {config['umap_path']}")
    ...
    for q_idx in queries:
        ...
        def get_slice(pid):
            ...
            img = nib.load(row["flair_path"])
            z = img.shape[2] // 2
            slice_data = np.array(img.dataobj[..., z], dtype=np.float32)
